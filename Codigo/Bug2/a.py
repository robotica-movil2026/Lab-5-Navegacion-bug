...
class Robot:
    def __init__(self):
        ...
        self.tactil = False
        self.ultrasonido = 0
        self.estado = ESTADO_SEGUIR_LINEA
    ...
    def seguir_linea(self, kp, error, potencia, umbral): #el kp es la variable de suma
        ...
        self.transiciones(ESTADO_FINALIZAR, self.meta != 0)
        self.transiciones(RETROCESO, self.tactil == 1)
        if self.tactil == 1:
    ...
    def giro_seguidor(self, potencia, angulo, umbral):

        if angulo > -45:
            self.izq = -potencia
            self.der = potencia
            self.estado = self.GIRO_SEGUIDOR
            return self.estado
        else:
            self.estado = self.ESTADO_SEGUIR_LINEA
            return self.estado
    ...
    def avance_ciego(self, potencia, ultrasonido_meta, umbral):
        self.izq = potencia
        self.der = potencia
